...
def parent_recipes(identifier):
    ...
    if recipe:
        ...
        display_verbose(f"List of recipe files: {pathlist}")
        return pathlist
    return []
...
def git_run(arglist):
    ...
    gitcmd = [GIT]
    gitcmd.extend(str(arg) for arg in arglist)
    # timeprint("Git cmd: %s" % gitcmd)
    results = run_cmd(gitcmd)
    if not results['success']:
        raise GitError(f"Git error: {results['stderr']}")
...
def branch_list():
    ...
    git_args = ['branch']
    if branch_output := git_run(git_args).rstrip():
...
def create_feature_branch(branch):
    ...
    display_verbose(f"Creating branch {branch}")
...
def change_feature_branch(branch, new=False):
    ...
    if USE_ARCANIST:
        arccmd = ['/usr/local/bin/arc', 'feature', branch]
...
def cleanup_branch(branch):
    ...
    display_verbose(f"Deleting branch {branch}: {results}")


def rename_branch_version(branch, version):
    """Rename a branch to include the version."""
    new_branch_name = branch + f"-{version}"
    if new_branch_name in branch_list():
        timeprint(f"Branch {new_branch_name} already exists")
        new_branch_name += '-2'
    gitcmd = ['branch', '-m', branch, new_branch_name]
    git_run(gitcmd)
    display_verbose(f"Renaming {branch} to {new_branch_name}")
...
def create_commit(imported_item):
    ...
    gitaddcmd = ['add', REPO_DIR]
    git_run(gitaddcmd)
    # Create the commit
    timeprint('Creating commit...')
    message = f"""Updating {str(imported_item['name'])} to version {str(imported_item["version"])}"""

    gitcommitcmd = ['commit', '-m', message]
...
def imported_task(imported_item):
    ...
    task_title = f"Package {imported_item['name']} has been updated in Munki."
    timeprint(f"Filing task: {task_title}")
    task_description = (
        "Catalogs: %s \n" % imported_item['catalogs']
        + "Package Path: %s \n" % imported_item['pkg_repo_path']
        + "Pkginfo Path: %s \n" % imported_item['pkginfo_path']
        + f"Version: {str(imported_item['version'])}"
    )
...
def failed_task(failed_items):
    ...
    for item in failed_items:
        task_title = f"Autopkg recipe {item['recipe']} failed to run."
        timeprint(f"Failure: {task_title}")
        task_description = f"Error: {item['message']}"
        create_task(task_title, task_description)
...
def run_recipe(recipe, report_plist_path, pkg_path=None):
    ...
    cmd = ['/usr/local/bin/autopkg', 'run', '-v', recipe]
    if pkg_path:
      cmd.append('-p')
      cmd.append(pkg_path)
...
def parse_report_plist(report_plist_path):
    ...
    if report_data['summary_results']:
        ...
        imported_items.extend(iter(munki_results.get('data_rows', [])))
    if report_data['failures']:
            # This means something went wrong
        failed_items.extend(iter(report_data['failures']))
    return {
...
def handle_recipe(recipe, pkg_path=None):
    ...
    display_verbose(f"Handling {recipe}")
    recipe_repo_dir = autopkglib.get_pref(
        'RECIPE_REPO_DIR'
    ) or os.path.expanduser('~/Library/AutoPkg/RecipeRepos')
...
def parse_recipe_list(file_path):
    ...
    if extension == '.json':
      ...
      raise RunlistError
    display_verbose(f"Recipe list: {recipe_list}")
    return recipe_list
...
if __name__ == '__main__':
    ...
    if args.recipes:
        ...
        display_verbose(f'Using argument recipes: {args.recipes}')
        runlist = args.recipes
        if len(runlist) == 1 and args.pkg:
            pkg_path = args.pkg
    elif passed_runlist:
      # Parse the list
